...
export async function POST(req: NextRequest) {
  ...
  for (const item of items) {
    ...
    // Calculer les semaines de fabrication en remontant depuis la livraison
    // Règle : logistique = semaine livraison, vitrage = -1 sem, montage = -2 sem, coupe = -3 sem
    // Pour SAV/Diffus (petits travaux) : tout compressé -1 sem
    ...
    const semVitrage = isSmall ? mondayLivraison : addWeeks(mondayLivraison, -1);
    const semMontage = isSmall ? addWeeks(mondayLivraison, -1) : addWeeks(mondayLivraison, -2);
    const semCoupe = isSmall ? addWeeks(mondayLivraison, -1) : addWeeks(mondayLivraison, -3);
