...
export default function PlanningAffectations({ commandes, viewWeek, onPatch, onWeekChange }: {
  ...
  const autoAssign = useCallback(() => {
    ...
    for (const del of Array.from(deliveries.values())) {
      ...
      // Priorité : livreurs non bookés. Fallback : n'importe quel livreur compétent
      // (un livreur peut faire une livraison courte + d'autres tâches la même demi-journée)
      ...
      // Ajouter aussi aux ops généraux pour satisfaire l'audit
    ...
    // Debug : afficher le résultat de la détection livraisons
    setTimeout(() => {
      const msg = `Livraisons détectées :\n` +
        `- ${nbTransporteurNous} commande(s) avec transporteur="nous" (toutes semaines)\n` +
        `- ${nbDansLaSemaine} commande(s) livrées cette semaine (S${weekId(viewWeek).slice(1)})\n` +
        `- ${deliveries.size} chargement(s) unique(s) par zone\n` +
        `- ${nbLivreursPlaces} chargement(s) avec livreurs assignés\n` +
        `- ${livreursDispo.length} livreur(s) disponible(s) : ${livreursDispo.map(o => o.nom).join(", ")}`;
      if (deliveries.size === 0 || nbLivreursPlaces === 0) alert(msg);
    }, 100);
